...
public class Fraud extends BasicClass {
   ...
   public Fraud() {
      ArrayList featureNameList = GetFeatureName.run(BeanFactoryConfig.configPlace.getZhenganConfigPlace().getFraud());
      Iterator var2 = featureNameList.iterator();

      while(var2.hasNext()) {
         String feature = (String)var2.next();
         this.features.put(feature, new BasicConfig(feature, BeanFactoryConfig.configPlace.getZhenganConfigPlace().getFraud()));
      }
