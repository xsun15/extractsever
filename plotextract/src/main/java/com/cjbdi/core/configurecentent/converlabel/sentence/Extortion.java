...
public class Extortion extends BasicClass {

   public Extortion() {
      ArrayList featureNameList = GetFeatureName.run(BeanFactoryConfig.configPlace.getSelfSentenceConfigPlace().getExtortion());
      Iterator var2 = featureNameList.iterator();

      while(var2.hasNext()) {
         String feature = (String)var2.next();
         this.features.put(feature, new BasicConfig(feature, BeanFactoryConfig.configPlace.getSelfSentenceConfigPlace().getExtortion()));
      }
