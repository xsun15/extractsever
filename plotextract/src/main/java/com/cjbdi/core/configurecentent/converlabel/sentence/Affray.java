...
public class Affray extends BasicClass {

   public Affray() {
      ArrayList featureNameList = GetFeatureName.run(BeanFactoryConfig.configPlace.getSelfSentenceConfigPlace().getAffray());
      Iterator var2 = featureNameList.iterator();

      while(var2.hasNext()) {
         String feature = (String)var2.next();
         this.features.put(feature, new BasicConfig(feature, BeanFactoryConfig.configPlace.getSelfSentenceConfigPlace().getAffray()));
      }
