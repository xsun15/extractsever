...
import com.cjbdi.core.extractcenter.sentence.individualfundraisingfraud.InitExtractor;
...
public class IndividualFundRaisingFraudExtractor extends LabelExtractor {

   private IndividualFundRaisingFraud individualFundRaisingFraud;
   private InitExtractor initExtractor = new InitExtractor();


   public IndividualFundRaisingFraudExtractor(IndividualFundRaisingFraud individualFundRaisingFraud) {
   ...
   public List extractPrivate(DefendantModel defendantModel, CasecauseModel casecauseModel) {
      ...
      while(var4.hasNext()) {
         ...
         Label label = basicSentenceFeatureClass.run(defendantModel, casecauseModel, this.individualFundRaisingFraud);
         if(label != null) {
            labelList.add(label);
         }
      }
