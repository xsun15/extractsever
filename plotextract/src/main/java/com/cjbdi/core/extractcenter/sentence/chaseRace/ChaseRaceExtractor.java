...
import com.cjbdi.core.extractcenter.sentence.chaseRace.InitExtractor;
...
public class ChaseRaceExtractor extends LabelExtractor {

   private ChaseRace chaseRace;
   private InitExtractor initExtractor = new InitExtractor();


   public ChaseRaceExtractor(ChaseRace chaseRace) {
   ...
   public List extractPrivate(DefendantModel defendantModel, CasecauseModel casecauseModel) {
      ...
      while(var4.hasNext()) {
         ...
         Label label = basicSentenceFeatureClass.run(defendantModel, casecauseModel, this.chaseRace);
         if(label != null) {
            labelList.add(label);
         }
      }
