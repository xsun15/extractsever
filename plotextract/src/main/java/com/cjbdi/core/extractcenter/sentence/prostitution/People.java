...
public class People extends BasicSentenceFeatureClass {
   ...
   public People() {
      this.name = ((ExtractFeatureBasicConfig)BeanFactoryConfig.extractFeatureConfig.getProstitution().getFeatures().get(this.code)).getName();
      this.positiveRule = ((ExtractFeatureBasicConfig)BeanFactoryConfig.extractFeatureConfig.getProstitution().getFeatures().get(this.code)).getPositivepurePattern();
      this.negativeRule = ((ExtractFeatureBasicConfig)BeanFactoryConfig.extractFeatureConfig.getProstitution().getFeatures().get(this.code)).getNegativepurePattern();
   }
   ...
   public Label run(DefendantModel defendantModel, CasecauseModel casecauseModel, BasicCaseClass caseClass) {
      ...
      if(fact.length() > 0) {
         ...
         if(nameSet.size() > 0) {
            Label label = SetLabel.run(this.code);
            return label;
         }
      }
