...
import com.cjbdi.core.extractcenter.sentence.seizing.InitExtractor;
...
public class SeizingExtractor extends LabelExtractor {

   private Seizing seizing;
   private InitExtractor initExtractor = new InitExtractor();


   public SeizingExtractor(Seizing seizing) {
   ...
   public List extractPrivate(DefendantModel defendantModel, CasecauseModel casecauseModel) {
      ...
      while(var4.hasNext()) {
         ...
         Label label = basicSentenceFeatureClass.run(defendantModel, casecauseModel, this.seizing);
         if(label != null) {
            labelList.add(label);
         }
      }
