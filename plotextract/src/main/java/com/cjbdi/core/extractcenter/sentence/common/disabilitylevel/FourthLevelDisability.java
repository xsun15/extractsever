...
public class FourthLevelDisability extends BasicSentenceFeatureClass {
   ...
   public FourthLevelDisability(String code) {
      this.init(code);
   }
   ...
   public void init(String code) {
      ...
      this.keyword = "伤残(等级)?[四4]级|[四4]级伤残";
      this.noiseword = "死|轻微伤|[(重伤)(轻伤)]|[(重伤)(轻伤)][一二12]级|伤残(等级)?[5-9一二三五六七八九十123]级|[5-9一二三五六七八九十123]级伤残";
   ...
   public void reset() {
      if(this.positivePurePattern != null && this.positivePurePattern.size() > 0) {
         for(int i = 0; i < this.positivePurePattern.size(); ++i) {
            ...
            this.positivePurePattern.set(i, Pattern.compile(line.replaceAll("占位", "四4")));
         }
      }
