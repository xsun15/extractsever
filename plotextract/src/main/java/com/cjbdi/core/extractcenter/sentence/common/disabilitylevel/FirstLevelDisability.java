...
public class FirstLevelDisability extends BasicSentenceFeatureClass {
   ...
   public FirstLevelDisability(String code) {
      this.init(code);
   }
   ...
   public void init(String code) {
      ...
      this.keyword = "伤残(等级)?[一1]级|[一1]级伤残";
      this.noiseword = "死|轻微伤|重伤|轻伤|((重伤)|(轻伤)[一二12]级)|伤残(等级)?[二三四五六七八九十2-9]级|[二三四五六七八九十2-9]级伤残";
   ...
   public void reset() {
      if(this.positivePurePattern != null && this.positivePurePattern.size() > 0) {
         for(int i = 0; i < this.positivePurePattern.size(); ++i) {
            ...
            this.positivePurePattern.set(i, Pattern.compile(line.replaceAll("占位", "一1")));
         }
      }
