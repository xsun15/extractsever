...
import com.cjbdi.core.extractcenter.sentence.invalidFraud.InitExtractor;
...
public class InvalidFraudExtractor extends LabelExtractor {

   private InvalidFraud invalidFraud;
   private InitExtractor initExtractor = new InitExtractor();


   public InvalidFraudExtractor(InvalidFraud invalidFraud) {
      ...
      this.invalidFraud = invalidFraud;
   }
   ...
   public List extractPrivate(DefendantModel defendantModel, CasecauseModel casecauseModel) {
      ...
      while(var4.hasNext()) {
         ...
         Label label = basicSentenceFeatureClass.run(defendantModel, casecauseModel, this.invalidFraud);
         if(label != null) {
            labelList.add(label);
         }
      }
