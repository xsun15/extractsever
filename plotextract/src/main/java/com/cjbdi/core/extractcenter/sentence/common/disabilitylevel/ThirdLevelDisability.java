...
public class ThirdLevelDisability extends BasicSentenceFeatureClass {
   ...
   public ThirdLevelDisability(String code) {
      this.init(code);
   }
   ...
   public void init(String code) {
      ...
      this.keyword = "伤残(等级)?[三3]级|[三3]级伤残";
      this.noiseword = "死|轻微伤|[(重伤)(轻伤)]|[(重伤)(轻伤)][一二12]级|伤残(等级)?[4-9一二四五六七八九十12]级|[4-9一二四五六七八九十12]级伤残";
   ...
   public void reset() {
      if(this.positivePurePattern != null && this.positivePurePattern.size() > 0) {
         for(int i = 0; i < this.positivePurePattern.size(); ++i) {
            ...
            this.positivePurePattern.set(i, Pattern.compile(line.replaceAll("占位", "三3")));
         }
      }
