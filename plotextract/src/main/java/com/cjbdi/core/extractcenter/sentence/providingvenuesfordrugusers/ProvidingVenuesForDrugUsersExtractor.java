...
import com.cjbdi.core.extractcenter.sentence.providingvenuesfordrugusers.InitExtractor;
...
public class ProvidingVenuesForDrugUsersExtractor extends LabelExtractor {

   private ProvidingVenuesForDrugUsers providingVenuesForDrugUsers;
   private InitExtractor initExtractor = new InitExtractor();


   public ProvidingVenuesForDrugUsersExtractor(ProvidingVenuesForDrugUsers providingVenuesForDrugUsers) {
   ...
   public List extractPrivate(DefendantModel defendantModel, CasecauseModel casecauseModel) {
      ...
      while(var4.hasNext()) {
         ...
         Label label = basicSentenceFeatureClass.run(defendantModel, casecauseModel, this.providingVenuesForDrugUsers);
         if(label != null) {
            labelList.add(label);
         }
      }
