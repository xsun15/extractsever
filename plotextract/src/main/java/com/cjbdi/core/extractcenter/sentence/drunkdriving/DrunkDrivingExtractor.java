...
public class DrunkDrivingExtractor extends LabelExtractor {

   private DrunkDriving drunkDriving;
   private InitExtractor initExtractor = new InitExtractor();


   public DrunkDrivingExtractor(DrunkDriving drunkDriving) {
   ...
   public List extractPrivate(DefendantModel defendantModel, CasecauseModel casecauseModel) {
      ...
      while(var4.hasNext()) {
         ...
         Label label = basicSentenceFeatureClass.run(defendantModel, casecauseModel, this.drunkDriving);
         if(label != null) {
            labelList.add(label);
         }
      }
