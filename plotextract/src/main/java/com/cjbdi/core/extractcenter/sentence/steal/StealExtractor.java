...
import com.cjbdi.core.extractcenter.sentence.steal.InitExtractor;
...
public class StealExtractor extends LabelExtractor {

   private Steal steal;
   private InitExtractor initExtractor = new InitExtractor();


   public StealExtractor(Steal steal) {
   ...
   public List extractPrivate(DefendantModel defendantModel, CasecauseModel casecauseModel) {
      ...
      while(var4.hasNext()) {
         ...
         Label label = basicSentenceFeatureClass.run(defendantModel, casecauseModel, this.steal);
         if(label != null) {
            labelList.add(label);
         }
      }
