...
public class ExtractCallable implements Callable {
   ...
   public Object call() {
      try {
         SentenceExtractor.initExtractor.basicInfo(this.defendantModel, this.casecauseModel);
         List e = ((LabelExtractor) SentenceExtractor.initExtractor.getSentenceExtractors().get(this.casecause)).extractPublic(this.casecauseModel);
         e.addAll(((LabelExtractor) SentenceExtractor.initExtractor.getSentenceExtractors().get(this.casecause)).extractPrivate(this.defendantModel, this.casecauseModel));
         List plotList = ((LabelExtractor) SentenceExtractor.initExtractor.getSentenceExtractors().get("总则量刑情节")).extractPublic(this.casecauseModel);
         plotList.addAll(((LabelExtractor) SentenceExtractor.initExtractor.getSentenceExtractors().get("总则量刑情节")).extractPrivate(this.defendantModel, this.casecauseModel));
         List courtDecisionList = ((LabelExtractor) SentenceExtractor.initExtractor.getSentenceExtractors().get("判决结果")).extractPrivate(this.defendantModel, this.casecauseModel);
